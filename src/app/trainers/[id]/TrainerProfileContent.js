...
import { 
    Star, MessageCircle, ChevronLeft, 
...
const iconMap = {
    ShieldCheck, Dumbbell, Target, Trophy, Award, GraduationCap, Heart
};
...
export default function TrainerProfileContent({ trainerId }) {
    ...
    const getTrainerInfo = () => {
        ...
        if (dbTrainer) {
            ...
            const found = Object.values(trainersData).find(t => {
                ...
                const dbAr = normalize(dbName.ar);
                const dbEn = normalize(dbName.en);
    ...
    const trainer = dbTrainer ? {
        name: getText(dbTrainer.name),
        coverImage: dbTrainer.coverImage || dbTrainer.image || (trainerInfo ? trainerInfo.coverImage : "/trainers/كابتن-وليد-عيسى_cover.png"),
        profileImage: dbTrainer.image || (trainerInfo ? trainerInfo.profileImage : "/trainers/default_trainer.png"),
        title: getText(dbTrainer.specialty) || (language === 'ar' ? 'مدرب محترف' : 'Professional Trainer'),
        specialty: getText(dbTrainer.specialty),
        rating: dbTrainer.rating || 5.0,
        reviewsCount: Math.floor(Math.random() * 100) + 10,
        experience: `${getText(dbTrainer.experience)} ${language === 'ar' ? 'سنوات' : 'Years'}`,
        location: dbTrainer.location || (language === 'ar' ? 'عن بعد' : 'Online'),
        bio: getText(dbTrainer.bio) || (language === 'ar' ? 'هذا المدرب لم يضف نبذة تعريفية.' : 'This trainer has not added a bio.'),
        stats: [
            { label: language === 'ar' ? 'تدريباً' : 'SESSIONS', val: '250+' },
            { label: language === 'ar' ? 'سنوات القوة' : 'YEARS EXP.', val: getText(dbTrainer.experience) || '1' },
            { label: language === 'ar' ? 'تقييم عام' : 'RATING', val: dbTrainer.rating?.toString() || '5.0' },
        ],
        achievements: (dbTrainer.achievements || []).map(a => ({
            title: getText(a),
            date: ''
        })),
        certificates: (dbTrainer.certificates || []).map(c => ({
            name: getText(c),
            issuer: ''
        })),
        specializations: (dbTrainer.expertise || []).map((sName, index) => ({
            id: index + 1,
            name: getText(sName),
            icon: Target
        })),
    ...
    if (!trainerInfo && !dbTrainer && !isLoading) {
        return (
            <div className={`min-h-screen ${darkMode ? "bg-[#0a0f1a]" : "bg-slate-50"} flex flex-col items-center justify-center p-8 text-center`}>
                <h2 className={`text-2xl font-bold ${darkMode ? "text-white" : "text-slate-900"} mb-4`}>
                    {language === 'ar' ? 'عذراً، لم يتم العثور على المدرب' : 'Sorry, Trainer Not Found'}
                </h2>
                <Link 
                    href="/trainers" 
                    className="bg-primary text-white px-8 py-3 rounded-xl font-bold hover:scale-105 transition-transform"
                >
                    {language === 'ar' ? 'العودة لقائمة المدربين' : 'Back to Trainers'}
                </Link>
            </div>
        );
    }
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="text-start">
                                                ...
                                                <div className="flex items-baseline gap-1">
                                                    ...
                                                    <span className="text-[10px] text-primary font-black uppercase">SAR</span>
                                                </div>
                                            </div>
